...
describe("buildTelegramMessageContext dm thread sessions", () => {
  ...
  it("uses thread session key for dm topics", async () => {
    ...
    expect(ctx?.ctxPayload?.SessionKey).toBe("agent:main:telegram:default:direct:1234:thread:42");
  });

  it("uses isolated dm session key when no thread id", async () => {
    const ctx = await buildContext({
      message_id: 2,
      chat: { id: 1234, type: "private" },
      date: 1700000001,
    ...
    expect(ctx?.ctxPayload?.SessionKey).toBe("agent:main:telegram:default:direct:1234");
  });
});
